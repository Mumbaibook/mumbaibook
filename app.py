...
def process_winning_bets(game_type, winning_numbers, date):
    try:
        # Standardize winning numbers to strings with leading zeros
        winning_numbers_standardized = [str(num).zfill(2) for num in winning_numbers]
        ...
        for bet in bets.data:
            # Standardize bet numbers
            bet_numbers_standardized = [str(num).zfill(2) for num in bet['numbers']]
            
            # Find matching numbers
            matching_count = len(set(bet_numbers_standardized) & set(winning_numbers_standardized))
            
            if matching_count > 0:
                # Use the existing potential_win value
                winnings = float(bet['potential_win'])
...
def get_results():
    try:
        ...
        for result in response.data:
            game_type_display = {
                'morning': 'Morning Game (21x)',
                'afternoon': 'Afternoon Game (11x)',
                'evening': 'Evening Game (5x)'
...
def place_oddeven_bet():
    try:
        ...
        current_balance = float(user_response.data[0]['balance'])
        if current_balance < amount:
            return jsonify({'error': f'Insufficient balance. Required: ₹{amount}'}), 400

        # Insert bet with empty numbers array
        bet_data = {
            'user_id': session['user_id'],
            'game_type': 'oddeven',
            'bet_type': bet_type,
            'numbers': [],  # Add empty array for numbers
            'amount': str(amount),
            'potential_win': str(amount * 2.1),  # Changed from 2.1 to 2.1
            'status': 'pending'
        }
        ...
        print(f"Error placing odd/even bet: {str(e)}")
        return jsonify({'error': str(e)}), 400

@app.route('/admin/add_oddeven_result', methods=['POST'])
@admin_required
def add_oddeven_result():
    try:
        data = request.json
        result = data.get('result')  # 'odd' or 'even'
        date = data.get('date')
        ...
        if not result or not date:
            return jsonify({'error': 'Missing required fields'}), 400

        if result not in ['odd', 'even']:
            return jsonify({'error': 'Invalid result type'}), 400

        # Check if result already exists for this date
        existing_result = supabase.table('oddeven_results')\
            .select('*')\
            .eq('date', date)\
            .execute()

        if existing_result.data:
            return jsonify({'error': f'Result already exists for {date}'}), 400
        ...
        response = supabase.table('oddeven_results').insert(result_data).execute()

        # Process winning bets
        process_oddeven_winning_bets(result, date)

        return jsonify({'message': 'Result added successfully'}), 200

    except Exception as e:
        print(f"Error adding odd/even result: {str(e)}")
        return jsonify({'error': str(e)}), 400

def process_oddeven_winning_bets(result, date):
    try:
        # Get all pending odd/even bets
        bets = supabase.table('bets')\
            .select('*')\
            .eq('game_type', 'oddeven')\
            .eq('status', 'pending')\
            .execute()
        ...
        for bet in bets.data:
            if bet['bet_type'] == result:
                ...
                # Update user balance with potential_win amount
                user = supabase.table('users')\
                    .select('balance')\
                    .eq('id', bet['user_id'])\
                    .single()\
                ...
                current_balance = float(user.data['balance']) if user.data['balance'] else 0
                winning_amount = float(bet['potential_win'])  # Use potential_win as winning amount
        ...
        print(f"Error processing odd/even winning bets: {str(e)}")
        raise

@app.route('/admin/oddeven-results')
@admin_required
def admin_oddeven_results():
    try:
        # Get all results for the last 7 days, ordered by date
        seven_days_ago = (datetime.datetime.now() - datetime.timedelta(days=7)).date().isoformat()
        
        response = supabase.table('oddeven_results')\
            .select('*')\
            .gte('date', seven_days_ago)\
            .order('date', desc=True)\
            .execute()

        # Calculate total bets and payouts for each result
        results = []
        for result in response.data:
            # Get bets for this result date
            bets_response = supabase.table('bets')\
                .select('status, amount, winning_amount')\
                .eq('game_type', 'oddeven')\
                .eq('date', result['date'])\
                .execute()

            total_bets = len(bets_response.data)
            total_payout = sum(float(bet['winning_amount'] or 0) 
        ...
        print(f"Error fetching odd/even results: {str(e)}")
        return jsonify({'error': str(e)}), 400

@app.route('/get_oddeven_results')
@login_required
def get_oddeven_results():
    try:
        date = request.args.get('date')
        if not date:
            date = datetime.datetime.now().date().isoformat()
